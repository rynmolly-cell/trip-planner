...
export default function TransportSegmentEditor({
  ...
  const segments = transport ?? [];

  const updateAt = (i: number, seg: TransportSegmentType) => {
    const next = [...segments];
    next[i] = seg;
    onChange(next);
  };
  ...
  const add = () => {
    onChange([...segments, { ...DEFAULT_SEGMENT }]);
  };
  ...
  return (
    <div className={styles.editor}>
      <span className={styles.editorLabel}>Transport between huts</span>
      {segments.map((seg, i) => (
        <div key={i} className={styles.segmentWrap}>
          <TransportSegment
            transport={seg}
            onChange={(t) => updateAt(i, t)}
            onRemove={segments.length > 1 ? () => removeAt(i) : undefined}
          />
        </div>
      ))}
      <button type="button" className={styles.addTransport} onClick={add}>
